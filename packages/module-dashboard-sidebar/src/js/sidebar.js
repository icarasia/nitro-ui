...
$(function() {
    ...
    menuItem.hover(function(){
     ...
     if(!$('.c-sidebar').hasClass('show')) {
         ...
         $('#'+thisSubmenu).animate({top: menuItemPos.top, left: sidebarWidth}, 0, 'linear').css({visibility: 'visible'}).addClass('is--open');
     }
    ...
    if(heightIncludeSubmenu >= windowHeight){
      submenu.css({top: submenuPosition});
    }
    ...
    menuItem.on('mouseleave', function(e){
      ...
      if($(e.relatedTarget).is(a)) {
          submenu.css({visibility: 'hidden'}).removeClass('is--open');
      }
    });
